$(document).ready(function(){
    ...
    */
    ...
    /* 
        ...
        오버한 li에 (header .gnb .gnb_wrap ul.depth1 > li)만 on 클래스 추가
    */
    $('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter focusin', function(){
        /* pc일때만 작동되어야함
           on이라는 명령은 이벤트핸들러(감시)로 pc에서만 작동(감시)되게 못함
           한번 감시를 붙이면 무조건 작동(감시)됨
           이벤트가 발생했을때 작동되는 실행문에 if문을 걸어서 pc일때만 실행되도록 해야함 */
        if(pc_mobile == 'pc'){
    ...
    /*
        모바일에서 1차 메뉴를 클릭하면
            - 1차 메뉴에 걸려있는 링크가 작동되면 안됨
            - 하위메뉴가 열려야함 (li에 open클래스를 추가)
              닫힌 1차 메뉴를 클릭하면 열리고
              열린 1차 메뉴를 클릭하면 닫힘
    ...
    */
    ...
    /*
        메뉴 열기를 클릭하면 header에 memu_open 클래스 추가
        메듀 닫기를 클릭하면 header애 memu_open 클래스 삭제
